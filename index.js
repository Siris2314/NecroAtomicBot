...
client.distube
    .on("playSong", (message, queue, song) => message.channel.send(
        message.channel.send(`${client.emotes.play} | Playing \`${song.name}\` - \`${song.formattedDuration}\`\nRequested by: ${song.user}\n${status(queue)}`)
    ))
    .on("addSong", (message, queue, song) => message.channel.send(
        message.channel.send(`${client.emotes.success} | Added ${song.name} - \`${song.formattedDuration}\` to the queue by ${song.user}`)
    ))

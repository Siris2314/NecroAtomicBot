  ...
  run: async (client, interaction) => {
    try {
      ...
      if (
        ...
        query.includes("https://open.spotify.com/track")
      ) {
        ...
        query.includes("https://open.spotify.com/playlist/") ||
        query.includes("https://www.youtube.com/playlist") ||
        query.includes("https://open.spotify.com/album")
      ) {
        ...
        Utils.search(query, null, queue, 10).then((songs) => {
          for (let i = 0; i < songs.length; i++) {
            arrsongs.push(songs[i].name);
          }

          let select = arrsongs
            .map((song, id) => `**${id + 1}**. ${song} `)
            .join("\n");

          const embed = new MessageEmbed()
            .setTitle(
              `Top searches for ${query}, please type in a number for me to play`
            )
          ...
          collector.on("collect", (message) => {
            if (isNaN(message.content)) {
              return interaction.channel.send(
                "Invalid Input, please re-run comand"
              );
            } else {
          ...
          collector.on("end", (collected) => {
            if (collected.size == 0) {
              interaction.channel.send(
                "Timed Out, Failed to Collect Song Input"
              );
              queue.stop();
            } else {
      ...
      return interaction.followUp({
        content: "Failed To Play Song, Error Code 403",
      });
    }
  },
};

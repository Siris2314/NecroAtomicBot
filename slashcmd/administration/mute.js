    ...
    run: async (client, interaction) => {
        ...
        if(!role) {
            try {
                ...
                interaction.followUp({content:'Muted role has sucessfully been created.'})
        ...
        if(Target.roles.cache.has(role2.id)) return interaction.followUp({content:`${Target.displayName} has already been muted.`})
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription(`✅ ${Target.user.username} has been muted for ${Reason}, duration of mute ${Time}`)
            .setColor("RED")
            .setTimestamp() 

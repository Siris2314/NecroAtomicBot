...
module.exports = {
  name: 'purgeword',
  description: 'Deletes any matched messages in the current channel!',
  ...
  async execute(message, args,client) {
    ...
    if (messages.size === 0) {
      ...
      message.channel.send(
        ...
        .setTitle('Purge')
        .setDescription(`
                Unable to find any messages with the text ${text}. 
      ...
      channel.bulkDelete(messages, true).then(messages => {
        const embed = new MessageEmbed()
          .setTitle('Purge')
          .setDescription(`
                Successfully deleted **${messages.size}** matching message(s). 
                This message will be deleted after \`10 seconds\`.
              `)
          .addField('Channel', channel, true)
          .addField('Message Count', `\`${messages.size}\``, true)
          .addField('Content to include:', `\`${text}\``, true)
          .setFooter(message.member.displayName, message.author.displayAvatarURL({
            dynamic: true
          }))
          .setTimestamp()
